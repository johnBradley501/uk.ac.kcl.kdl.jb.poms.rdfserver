...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<html xml:lang="en" lang="en">
			<head>
				...
				<base href="/rdf/repositories/poms/" />
			...
			<body>
				<div id="header">
					...
					<table>
						...
						<tr>
							<td><a href="../../ontology/doc" class="chlink">POMS Ontology Documentation</a></td>
						</tr>
					</table>
				</div>
					<div id="logo">
						
						<h1 class="mainHeader">People of Medieval Scotland: RDF Server</h1>
					</div>
				</div>
				<div id="navigation">
					<ul class="maingroup">
	...
	<xsl:template name="navigation-explore">
		...
		<xsl:call-template name="navigation-entry">
			<xsl:with-param name="label" select="$classes.label" />
			<xsl:with-param name="href" select="'classes'" />
			<xsl:with-param name="disabled" select="$disabled" />
		</xsl:call-template>
		<xsl:call-template name="navigation-entry">
			<xsl:with-param name="label" select="$properties.label" />
			<xsl:with-param name="href" select="'properties'" />
			<xsl:with-param name="disabled" select="$disabled" />
		</xsl:call-template>
		<xsl:call-template name="navigation-entry">
