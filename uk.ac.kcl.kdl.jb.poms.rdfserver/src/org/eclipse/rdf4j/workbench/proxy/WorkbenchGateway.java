 ...
 * repository manager held by uk.ac.kcl.kdl.jb.rdf.server.SharedRepositoryHandler. The only changes
 * are in getDefaultServerPath() and init().    ... jb June 2017
 */
public class WorkbenchGateway extends AbstractServlet {
	...
	{
		super.init(config);
		SharedRepositoryHandler.initInstance(config); // added to rdf4j original   JB
	...
	public String getDefaultServerPath() {
		//return config.getInitParameter(DEFAULT_SERVER);   // original code from rdf4j   JB
		String parm = SharedRepositoryHandler.getServerURL();
		return parm;
	}
